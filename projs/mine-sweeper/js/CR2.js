// I fixed the bug and handle the victory 
...
function buildBoard() {
    ...
    //CR : DOUBLE ASSIGNING, BOTH ASSIGNING IT TO gMat AND THEN RETURNING THE BOARD USING IT IN THE FUNC
    gMat = board;
    return board;
}
...
function placeMines(mat, i, j) {
    ...
    while (minesPlaced < MINES) {
        ...
        if (jRnd === j) jRnd = getRandomNumber(SIZE);
...
function renderBoard(board) {
    ...
    var strHTML = '<table border="1"><thead><tr border="0"><th>Mines: ' + MINES + '</th><th colspan="' + tableSize + '" class="game"><img src="img/regular.png" class="smiley" onclick="initGame()"><span class="timer">0</span></th></tr></thead><tbody>';
    for (var i = 0; i < board.length; i++) {
        strHTML += '<tr>';
        for (var j = 0; j < board[0].length; j++) {
            var cell = board[i][j];
...
function checkGameOver() {
    ...
    var boardSize = SIZE * SIZE - MINES;

    if (markedCells === MINES && shownCells === boardSize) {
        youWon();
    }
}
...
function revealCell(i, j) {
    ...
    elCell.classList.remove('hidden-grey');
    if (gMat[i][j].isMine === true) {
        var className = '.cell' + i + '-' + j; //CR :YOU ALREADY CREATED THIS VAR
        var elCell = document.querySelector(className); //CR : ALREADY ASSIGNED THIS AS WELL
...
function expandShown(coordI, coordJ) {
    //CR : VERY COMPLICATED, DOESN'T HAVE TO BE, ALSO DOESN'T WORK MOST OF THE TIMES
    var res = [];
    var i = coordI;
...
function youWon() {
    ...
    gameOver = true;
    stopTimer();
    setTimeout(smileyWon, 200);
    gGame.isOn = false;
    alert("Victory!");
}

function isMine(i, j) {
    //CR : REALLY NO NEED TO EXPORT THIS TO A DIFFERENT FUNCTION
    //CR : ALSO A BETTER NAME WOULD BE checkIsMine
    if (gMat[i][j].isMine === true) {
        return true;
    }
}
